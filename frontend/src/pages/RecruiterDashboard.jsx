...
export default function RecruiterDashboard() {
    ...
    <div className="w-full max-w-screen-md mx-auto px-4 py-6 space-y-10">
      ...
      <div className="flex justify-center gap-4 mb-6 flex-wrap">
        <button
          ...
          className={`px-5 py-2 rounded-full text-sm font-semibold ${
            activeSection === 'upload' ? 'bg-accent text-black' : 'bg-gray-700 text-white hover:bg-gray-600'
          }`}
        >
          📤 Upload JD
        </button>
        <button
          ...
          className={`px-5 py-2 rounded-full text-sm font-semibold ${
            activeSection === 'resume' ? 'bg-accent text-black' : 'bg-gray-700 text-white hover:bg-gray-600'
          }`}
        >
          🔄 Resume → JD
        </button>
        <button
          ...
          className={`px-5 py-2 rounded-full text-sm font-semibold ${
            activeSection === 'onetoone' ? 'bg-accent text-black' : 'bg-gray-700 text-white hover:bg-gray-600'
          }`}
        >
          🔗 One-to-One
        </button>
      ...
      {activeSection === 'upload' && (
        <section className="bg-[#151515] border border-gray-700 rounded-2xl p-8 shadow-lg space-y-6">
          <h2 className="text-xl font-bold text-accent">Upload Job Description & Resumes</h2>
  ...
  {jdFile && <p className="text-sm text-green-400 mt-2">✅Uploaded: {jdFile.name}</p>}
</div>
            <div className="relative w-full">
  <label className="bg-gray-800 text-white border border-gray-600 px-4 py-2 rounded cursor-pointer inline-block w-full text-center hover:bg-gray-700 transition">
    📎 Choose Resume File
  ...
  {resumeFile && <p className="text-sm text-green-400 mt-2">✅Uploaded: {resumeFile.name}</p>}
</div>
